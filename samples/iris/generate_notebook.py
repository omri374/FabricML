...
@click.command(name="Generate a new notebook based on a markdown template "
                    "(found in notebook_templates/notebook_template.md")
@click.option('--name', help='The name of the notebook to be generated')
@click.option('--path', help='Folder in which the notebook will be generated in', default="notebooks")
@click.option('--template_file', help='Template to generate notebook from (from the notebook_templates folder)',
...
def generate_notebook(template_file, name, path="notebooks", file_type='ipynb'):
    ...
    print(f"Saving notebook {name} to {absolute_notebooks_path}")
